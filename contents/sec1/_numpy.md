...
と書く。この時、`numbers`の各要素を2倍にしたいとすると、直感的には
...
のように配列が2回繰り返されたものとなってしまう。Python のリストでこれを実現するには**リスト内包表記**を使って
...
NumPyを用いて配列を作る場合、Pythonのリストを引数にとる関数`np.array`を用いて、
...
また、NumPyには、単に直感的に書ける、というだけでなく計算速度の面でも有利な点がある。

NumPyの配列は、通常、配列の上の要素が全て同じ型 (`int`や`float`など) を持っており、メモリ上でも連続した領域にデータが格納されている。

NumPyはC言語を含む低レベルのプログラミング言語で実装されているため、上記の特徴を持つデータに対して、Pythonのループを使用するよりも遥かに高速な計算が可能である。

このように、要素ごとのループを明示的に書く代わりに、配列全体に対する演算として計算を記述することを**ベクトル化**と呼ぶ。

ベクトル化を上手く使ってプログラムを書くには少々コツが必要だが、それができれば、Pythonのループを使用する場合に比べて高速なプログラムの実装ができる。
...
Pythonのリストで多次元配列を作るには、

```{code-cell} ipython3
numbers = [[1, 2], [3, 4]]
```

のように`[ ]`を二重にすれば良かった。同様の配列をNumPyで作るにはいくつかの方法がある。最も単純には、上記の配列を`np.array`の引数に設定すれば良い。
...
また、全ての要素が0の10 $\times$ 10の二次元配列を作りたい場合、Pythonのリストでは、リスト内包表記を用いて
...
一方、NumPy で同様の配列を作る場合、先ほど紹介した方法で`np.array`の引数に Python の多次元リストを代入する以外に`np.ndarray`というクラスを直接使う方法がある。

`np.ndarray`は配列のサイズをタプルとして引数に取るので、単に10x10の配列を作りたい場合には、引数に `(10, 10)`を指定すれば良い。

```{code-cell} ipython3
arr = np.ndarray((10, 10))
```

また、全要素が0の10 $\times$ 10の二次元配列をNumPyで作る場合には `np.zeros` を用いる。
...
`np.ndarray`は配列のためのメモリ領域を確保するだけで、その中身を初期化しない。そのため、上記の`zeros`という変数には、実際には 0 ではない値が入っている可能性がある。

中身が0で初期化された配列が欲しい場合には、次に紹介する`np.zeros`を使うこと (逆に、中身を初期化しないことを明示したい場合には`np.empty`を使うのが一般的である)。
:::
...
上記の関数を使う場合、`np.ndarray`の時と同様、関数の引数が多次元配列の大きさを表す**タプル**になるので注意すること。
...
```{code-cell} ipython3
print(np.arange(10))  # 0以上, 10未満について1刻み
```

```{code-cell} ipython3
print(np.arange(5, 10))  # 5以上, 10未満について1刻み
```

```{code-cell} ipython3
print(np.arange(0, 10, 2))  # 0以上, 10未満について2刻み
```
...
`np.linspace`を使うことで、とある上限と下限の間に、指定した個数の等間隔な値を生成することもできる。このとき、上端を含むか、含まないかを`endpoint`引数により指定することもできる(初期値は`True`)。

```{code-cell} ipython3
print(np.linspace(0, 10, 5))  # 両端点を含み0.0と10.0の間から5点
```

```{code-cell} ipython3
print(np.linspace(0, 10, 5, endpoint=False))  # 上端を含まず、0.0と10.0の間から5点
```

:::{admonition} 練習問題
:class: question

全ての要素が $-1$ である5 $\times$ 5の配列を、`np.full`を使う方法と、`np.ones`を使う方法の2通りで作りなさい。
:::
...
`np.arange`と`np.linspace`のそれぞれを用いて、0.0から1.0までを0.05刻みで並べた要素数21の配列を作りなさい。
...
上記のNumPyの配列をJupyter Notebook上で表示すると
...
NumPyは整数と浮動小数については配列内の要素が既定の型で表されている場合にその表示を省略するため、配列の中の整数が実際に`int32`型や`int64`型などの如何なる型で表されているかはこれだけでは分からないことがある。

配列内の要素の型を調べるには、明示的に配列の`dtype`フィールドにアクセスすれば良い。
...
のようにすることもできる。上記の例では、配列内の各要素が`float32`型、すなわち 32bitの単精度浮動小数で表わされる。

NumPy で使える配列の型には、整数系 (`int32` / `uint32`など)、浮動小数系 (`float32` / `float64`など)、複素数形 (`complex64`: 実部と虚部がそれぞれ32bit浮動小数)などがある。

また、最初に異なる型で宣言した配列を途中から別の型に変更したい場合には`astype`メソッドを使えば良い。
...
このようなクラスにおいて、クラスが保持する変数 (= `self.name`や`self.age`)のことを**属性**や**フィールド**と呼び、クラスが備える関数(= `intro`)のことを**メソッド**と呼ぶ。
...
これらの用語はPythonに限らず、オブジェクト指向型言語の基本用語なので覚えておくこと。
:::
...
上記の通り、flatten と ravel はほとんど同じ機能を提供するが、得られる戻り値が、元の配列をコピーした「別の実体」なのか、あるいは、元の配列と要素データを共有する一次元配列的なのか、という点が異なる。前者をコピー、後者を**ビュー**と呼ぶ。
...
また、NumPy の配列は、次元を増やすことも可能で、例えば 2x2 の二次元配列を 2x1x2 の三次元配列にしたりできる。これには`reshape`を使うことができるほか、新しく追加する次元に対応する箇所に`None`あるいは`np.newaxis`と書くことで次元を追加できる。
...
の例では、`a`の各要素に1が足し算されていることが分かる。これは、他の四則演算に対しても同様である。
...
ただし、NumPyには後述する**ブロードキャスト**という仕組みがあり、`shape`が異なる要素同士であっても一定の条件下で二項演算ができる。この場合には、予期しない要素同士が演算される可能性があるので注意すること。
...
二次元ベクトルを引数に取る`np.cross`は NumPy 2.0 以降では非推奨となっており、実行すると`DeprecationWarning`が表示される (将来のバージョンでは削除される予定である)。

この計算の実体は 2x2 行列の行列式であるから、二次元の場合には
...
前項では、 $\mathbf{Ax} = \mathbf{b}$ という線形方程式に対して、逆行列を計算して解 $\mathbf{x}$ を求める方法について述べた。
...
:::{admonition} 連立一次方程式と行列
:class: note

$N$ 個の未知数 $x_1, \ldots, x_N$ を要素とするベクトル $\mathbf{x}$ についての $M$ 本の一次方程式は、係数を並べた $M \times N$ 行列 $\mathbf{A}$ と、右辺の値を並べた $M$ 次元ベクトル $\mathbf{b}$ を用いて $\mathbf{Ax} = \mathbf{b}$ と書ける。

このとき、方程式の解は、次の3つのいずれかになる。
...
ここで $[\mathbf{A}\ \mathbf{b}]$ は $\mathbf{A}$ の右側に $\mathbf{b}$ を列として付け加えた $M \times (N+1)$ 行列である。

1番目は、$\mathbf{A}$ の $N$ 本の列ベクトルが一次独立で、かつ $\mathbf{b}$ がそれらの線形結合として一意に表せる場合に対応する。

`numpy.linalg.solve`が扱えるのは、このうち1番目の場合で、特に、 $\mathbf{A}$ が正方行列 ($M = N$) かつ正則な場合に限られる。

2番目のように解が定まらない場合や、3番目のようにそもそも解が存在しない場合をどう扱うかは、後述の [発展: 擬似逆行列と最小二乗問題](ssec:least-squares-and-pseudo-inverse) で改めて取り上げる。
:::
...
特に画像を取り扱う場合など、画素の座標を取得するために、二次元の離散的な座標系を作成したいことがよくある。

例えば`ix`は`[[1, 2, ..., 10], ..., [1, 2, ..., 10]]`、`iy`は`[1, 1, ..., 1], [2, 2, ...., 2], ..., [10, 10, ..., 10]]`といった形だ。このような場合には`[1, 2, ..., 10]`という配列を作成した上で`np.meshgrid`を用いると良い。
...
また、NumPy には`np.mgrid`という変数が用意されていて、これは指定した範囲から`meshgrid`で得られるような座標配列を生成するオブジェクトである。`np.mgrid`を用いると、`np.arange`を用いることなく二次元座標系を作成することができる。
...
(ssec:least-squares-and-pseudo-inverse)=
### 発展: 擬似逆行列と最小二乗問題

+++

正則でない行列 (= **特異行列**という)には逆行列は存在しない。ここでは、特異行列を係数行列に持つ連立一次方程式や最小二乗問題の扱いについて見てみる。
...
上記の例では、例外処理をしており、**LinAlgError**が発生して、その例外メッセージが**Singular matrix**となることが確認できる。実際、上記の行列のランクは1で行列式は0.0である。

さて、このような特異行列に対して、逆行列を求めるにはどうすれば良いのだろうか？特異行列を係数行列に持つ連立一次方程式を解きたいという場面は多く、その一番身近な例が、以下のような最小二乗問題を解きたい場合だろう。
...
この場合、 $\mathbf{x}$ について目的関数を微分したものがゼロベクトルになる箇所を探せば、それが求める $\mathbf{x}$ である。これは、目的関数の $\mathbf{x}$ についての微分によって得られる以下の **正規方程式** を解くことに相当する。

$$
\mathbf{A}^\top \mathbf{Ax} = \mathbf{A}^\top \mathbf{b}
$$

しかし、実用上、 $\mathbf{A}^\top \mathbf{A}$ が特異行列であることも多い。

+++

この場合、上記の最小化問題の解は、特異行列 $\mathbf{A}$ に対して逆行列と似た性質を持つ**擬似逆行列** (正しくはMoore-Penroseの擬似逆行列と呼ぶ) を右辺に乗ずることで求められる。

擬似逆行列を $\mathbf{A}^\dagger$ のように表す時、最小化を実現する $\mathbf{x}^\ast$ は次の式で与えられる。

$$
\mathbf{x}^\ast = \mathbf{A}^\dagger \mathbf{b}
$$

ただし、擬似逆行列を経由して得られる $\mathbf{x}^\ast$ は、行列 $\mathbf{A}$ の性質によって、その意味が変化する。

行列 $\mathbf{A} \in \mathbb{R}^{M \times N}$ について、 $M > N$ であって、$\text{rank} \mathbf{A} = N$ の場合、すなわち、未知数の数が式の数より少ない場合には、正規方程式に現れる $N \times N$の正方行列$\mathbf{A}^\top \mathbf{A}$ が正則行列になるため、最小二乗解は一意であり、従って擬似逆行列を用いて得られる解も最小二乗解となる。

一方、 $M < N$ であって、 $\text{rank} \mathbf{A} = M$ の場合、すなわち、未知数の数が式の数より多い場合には、正規方程式に現れる $N \times N$ の正方行列 $\mathbf{A}^\top \mathbf{A}$ のランクは $M$ である。従って、 $\mathbf{A}^\top \mathbf{A}$ は特異行列となり、最小二乗解は無数に存在することになる。

結論を先に述べると $\mathbf{x}^\ast$ は、最小二乗解の中でノルムが最小の解となる。その理由は以下のようにして示される。

$M < N$ の場合、擬似逆行列は $\mathbf{A}^\dagger = \mathbf{A}^\top (\mathbf{A} \mathbf{A}^\top)^{-1}$ と表せる。従って、

$$
\mathbf{x}^\ast = \mathbf{A}^\top (\mathbf{A} \mathbf{A}^\top)^{-1} \mathbf{b}
$$

である。今、$\mathbf{Ax} = \mathbf{b}$ を満たす$\mathbf{x}$は無数に存在するので、任意の解$\mathbf{x}$ について、

$$
\mathbf{x}^\ast = \mathbf{A}^\top (\mathbf{A} \mathbf{A}^\top)^{-1} \mathbf{Ax}
$$ (eq:x-ast-equation)

である。この $\mathbf{x}$ について、 $\| \mathbf{x} \|^2$ を考えると、

$$
\begin{aligned}
\| \mathbf{x} \|^2
&= \| \mathbf{x} - \mathbf{x}^\ast + \mathbf{x}^\ast \|^2 \\
&= \| \mathbf{x} \|^2 + \| \mathbf{x}^\ast \|^2 + 2 (\mathbf{x} - \mathbf{x}^\ast)^\top \mathbf{x}^\ast
\end{aligned}
$$

が得られる。この時、第3項の $(\mathbf{x} - \mathbf{x}^\ast)^\top \mathbf{x}^\ast$ は{eq}`eq:x-ast-equation` を用いると0になる。従って、

$$
\| \mathbf{x} \|^2 = \| \mathbf{x}^\ast \|^2 + \| \mathbf{x} - \mathbf{x}^\ast \|^2
$$

であるが、 $\| \mathbf{x} - \mathbf{x}^\ast \|^2$ は非負であるため、 擬似逆行列から求まる $\mathbf{x}^\ast$ は、最小二乗解の中でノルムが最小の解であることが分かる。
...
正則行列に対する逆行列と、特異行列に対する擬似逆行列の違いと共通点について述べよ。
...
コンピュータによる数値計算では、数学的には特異である(＝行列式が0である)ような行列が、数値誤差の影響で非常に小さな非ゼロの行列式を持つために正則と判断されてしまうことがある。そのため、実際の問題で逆行列を計算するときには、注意が必要である。多くの場合は、逆行列の代わりに擬似逆行列を用いることで上記の問題を回避できるが、その場合には擬似逆行列を用いて得られる解が、どのような性質を持つかに留意する必要がある。

擬似逆行列を用いる、ということは行列が特異、すなわちランク落ちしている場合であるから、実際の線形方程式を満たす解は無数に存在することになる。擬似逆行列を用いて得られる解は、そのうち、ノルムの大きさ (より正確にはl2ノルムの大きさ)が最小になるものになるので、自身が求めたい解がその解で良いのかは、実際の問題を扱う上では重要だろう。
...
このような場合、一度LU分解を計算してしまって、三角行列に対する線形問題を解く、という方法が考えられる。残念ながら、この方法は NumPy には実装されていないが類似ライブラリである SciPy を用いると、以下の形で実現できる。
...
この結果において`lu`は上三角行列と下三角行列を結合した行列であり、`piv`はピボット選択によって生じた行の入れ替わり (連立方程式において方程式の順序を変える操作に対応)を表わす自然数の配列である。ピボット選択については、数値計算の初等的な教科書にも書かれている内容なので、各自、教科書を読むなどして勉強してみてほしい。
...
**Jacobi法**は**対称行列**に対するアルゴリズムで、Givens回転と呼ばれる行列の一部要素だけを変換するような下記の行列 $\mathbf{G}$ を用いる (値の入っていない箇所の要素は全て 0)。
...
このとき、固有値を求めたい行列 $\mathbf{A}$ の $i$ 行 $j$ 列の要素を $a_{ij}$ と記すことにするとし、上記のGivens回転の回転量 $\theta$ を

$$
\tan 2\theta = \frac{-2a_{ij}}{a_{ii} - a_{jj}}
$$

となるように設定する。ちなみに実装上は $a_{ii} = a_{jj}$ の場合もあるので、

```python
theta = 0.5 * np.arctan2(-2 * a_ij, a_ii - a_jj)
```

のように求めると良い。Givens回転を$\mathbf{G}_k$を

$$
\mathbf{A}_{k+1} = \mathbf{G}_k \mathbf{A}_k \mathbf{G}_k^\top
$$

のように適用すると、$\mathbf{A}_k$ の $a_{ij} = a_{ji}$ の要素を 0 にすることができる。

この操作を $\mathbf{A}_0 = \mathbf{A}$ から始めて、行列 $\mathbf{A}_k$ の非対角成分のうち最も絶対値が大きい物が十分小さくなるまで繰り返すことで、$\mathbf{A}_k$ が固有値を対角成分に持つ行列 $\pmb{\Lambda}$ へと収束していく。

また、Givens回転行列は直交行列であるため、元の行列 $\mathbf{A}$ を $\pmb{\Lambda}$ に変換する過程で用いた全てのGivens回転の積として得られる行列

$$
\mathbf{P} = \mathbf{G}_{K-1} \mathbf{G}_{K-2} \cdots \mathbf{G}_0
$$

の各行が固有ベクトルとなり、

$$
\pmb{\Lambda} = \mathbf{P} \mathbf{A} \mathbf{P}^\top
$$
...
**QR法**はQR分解を用いて実対称行列の固有値・固有ベクトルを求めるアルゴリズムである。

一般の対称とは限らない実行列 $\mathbf{A}$ が、QR分解によって $\mathbf{A} = \mathbf{QR}$ (ただし、Q は直交行列、R は上三角行列)と分解されるとすると、

$$
\mathbf{A}' = \mathbf{Q}^\top \mathbf{A} \mathbf{Q} = \mathbf{Q}^\top \mathbf{Q} \mathbf{RQ} = \mathbf{RQ}
$$

のように行列 $\mathbf{Q}$ と $\mathbf{R}$ の順序を入れ替えるような操作を繰り返すと $\mathbf{A}$ は徐々に**対角成分に固有値が並ぶ上三角行列**へと近づいていく (この形を実Schur形と呼ぶ)。

特に $\mathbf{A}$ が実対称行列である場合には、$\mathbf{Q}^\top \mathbf{A} \mathbf{Q}$ という変換が対称性を保つため、収束先は固有値を対角成分に持つ対角行列 $\pmb{\Lambda}$ となる。

この場合には、変換の過程で得られる全ての直交行列 $\mathbf{Q}$ の積を取ることで、固有ベクトルを各列に持つような直交行列が得られる。

一方、非対称行列の場合には、同じ積によって得られるのはSchurベクトルと呼ばれる別のベクトルであり、そのままでは固有ベクトルにはならないことに注意してほしい。
